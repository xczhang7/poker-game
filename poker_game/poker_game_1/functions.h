...
void print_vec(vector<int>play){
    cout<<endl;
    B_sort(play);
    for(int i=0;i<play.size();++i){
      if(play[i]<52){
        ...
        if(play[i]%13==0){cout<<"A ";}
        else if(play[i]%13>0 && play[i]%13<10){cout<<play[i]%13+1<<' ';}
        else if(play[i]%13==10){cout<<"J ";}
        else if(play[i]%13==11){cout<<"Q ";}
        else if(play[i]%13==12){cout<<"K ";}
      }
      else
        switch(play[i]){
          case 52:cout<<"B_Joker ";
          case 53:cout<<"R_Joker ";
        }
    }
    cout<<'\n'<<endl;
    return;
...
bool N_inVec(int num,vector<int>obj){
  for(int i=0;i<obj.size();++i){
...
void rem(vector<int>now,vector<int>&hand){
  for(int l=0;l<hand.size();++l){
    if(N_inVec(hand[l],now))
...
void remVec(string fileName, vector<int>obj){
    ...
    for(int i=0;i<obj.size();++i){
      if(obj[i]<52){
        ...
        switch(obj[i]){
          case 52:itr="B_Jack";break;
          case 53:itr="R_Jack";break;
        }
      }
    obj1.push_back(itr);
    }
...
//这个是判断出牌合不合理的函数。
bool validPlay(vector<int>last,vector<int>now,vector<int>hand){
  for(int y=0;y<now.size();y++){
    if(!N_inVec(now[y],hand))
      return false;
  }
